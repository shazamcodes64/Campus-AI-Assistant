...
Tests concurrent sessions with pre-initialized engines to avoid threading issues
...
class PerformanceTester:
    ...
    def __init__(self, config_path="config.json"):
        ...
        # Pre-initialize engines (singleton pattern)
        print("🔧 Pre-initializing engines...")
        ...
        self.ollama_available = self.llm_generator.test_connection()
        if not self.ollama_available:
            print("⚠️  Ollama not available - LLM generation will be skipped")
        else:
            print("✅ Engines initialized (Ollama connected)")
    ...
    def single_query_test(self, query: str, user_id: int) -> bool:
        ...
        try:
            # Use pre-initialized engines (thread-safe)
            search_start = time.time()
            search_result = self.search_engine.search(query)
            search_time = time.time() - search_start
            
            # Only call LLM if Ollama is available
            if self.ollama_available:
                llm_start = time.time()
                response = self.llm_generator.generate_response(query, search_result)
                llm_time = time.time() - llm_start
            else:
                # Skip LLM generation if Ollama not available
                llm_time = 0.0
                response = {
                    "confidence": search_result.get("confidence", 0),
                    "method": "search_only_no_llm",
                    "sources": search_result.get("result", {}).get("sources", [])
                }
    ...
    def analyze_results(self):
        ...
        all_passed = self._assess_performance()
...
def main():
    ...
    # Analyze results
    all_passed = tester.analyze_results()
