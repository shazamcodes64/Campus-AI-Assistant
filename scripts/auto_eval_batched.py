...
BATCH_SIZE = 8  # Process queries in batches for embedding optimization
...
print(f"Running batched automated torture test on {len(QUERIES)} queries...")
print(f"Batch size: {BATCH_SIZE} (embeddings computed in parallel)")
...
with open("eval_results_batched.csv", "w", newline="") as f:
    writer = csv.writer(f)
    ...
    for batch_start in range(0, len(QUERIES), BATCH_SIZE):
        ...
        print(f"\n=== Batch {batch_start // BATCH_SIZE + 1}/{(len(QUERIES) + BATCH_SIZE - 1) // BATCH_SIZE} ===")
        print(f"Processing queries {batch_start + 1}-{batch_end}...")
        
        batch_start_time = time.time()
        
        try:
            # Batch search (embeddings computed in parallel)
            search_results = engine.search_batch(batch_queries)
            
            batch_embedding_time = time.time() - batch_start_time
            print(f"  ✓ Batch embedding + search: {batch_embedding_time:.2f}s")
            
            # Process each result individually (LLM calls still sequential)
            for idx, (q, search_result) in enumerate(zip(batch_queries, search_results)):
                query_num = batch_start + idx + 1
                print(f"\n[{query_num}/{len(QUERIES)}] {q[:60]}{'...' if len(q) > 60 else ''}")
            ...
            # Write error rows for all queries in batch
...
print("\nSaved detailed log → eval_results_batched.csv")
print("\n💡 Batched embedding computation improves throughput for multiple queries")
